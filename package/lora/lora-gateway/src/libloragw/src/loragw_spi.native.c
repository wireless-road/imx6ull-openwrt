...
//printf("lgw_spi_w. Sending: [%08X] [%08X] [%08X]\n", out_buf[0], out_buf[1], ( (command_size == 3 ) ? out_buf[2] : 0) );
...
//printf("lgw_spi_r. Sending: [%08X] [%08X] [%08X]\n", out_buf[0], out_buf[1], ( (command_size == 3 ) ? out_buf[2] : 0) );
...
int lgw_spi_wb(void *spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, uint8_t address, uint8_t *data, uint16_t size) {
    ...
    size_to_do = size;
/*
printf("lgwi_spi_wb. Sending: [%08X] [%08X]\n", command[0], ( (command_size == 2 )? command[1] : 0) );
	for(i=0;i<size;i++){
		if(i % 7)
			printf(" [%08X]", data[i]);
		else
...
//printf("lgwi_spi_rb. Sending: [%08X] [%08X]\n", command[0], ( (command_size == 2 )? command[1] : 0) );
//	for(i=0;i<size;i++){
//		if(i % 7)
//			printf(" [%08X]", data[i]);
//		else
